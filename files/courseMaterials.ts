...
import { CreateCourseMaterialSchema, UpdateCourseMaterialSchema } from "../schemas";
...
router.post(
  "/",
  authenticate,
  validateRequestBody(CreateCourseMaterialSchema),
...
router.put(
  "/:id",
  authenticate,
  validateRequestBody(UpdateCourseMaterialSchema),
  async (req: AuthRequest, res: Response) => {
    try {
      const material = await prisma.courseMaterial.findFirst({
        where: { id: req.params.id, user_id: req.user_id },
